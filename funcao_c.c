...
int Voos(int *assentosReservados[50][10]) // Funcao para verificar a ocupação de cada voo
{
...
int imprimir(int *assentosReservados[50][10],int voo, int numero_voo) // Função para imprimir reservas de um voo especifico.
{
    ...
    switch(resposta)
    {
        case 1:
            ...
            if (consulta >=0 && consulta < 50)
            {
                ...
                imprimir(assentosReservados, consulta, voo);// Chama a função para imprimir as reservas.
            }
        break;
...
int assentosVagos (int iInf, int *assentosReservados[50][10]) //Função para exibir as vagas de um voo apenas.
{
...
void mostrarInclu (int voo, int *assentosReservados[50][10])  //Função para mostrar os dados de apenas um voo.
{
    int j, lug = 1;
    for (j=0; j<10; j++)
    {
...
int incluirReserva(int cpf, int voo, int assentoEscolhido, int *assentosReservados[50][10]) //Função de incluir reservas a partir dos dados fornecidos.
{
    int j;
    assentoEscolhido --;
    for (j=0; j<10; j++)
    ...
    while (opcaoEscolhida != 0)
    {
        ...
        case 2:
            ...
            if (voo >=0 && voo < 50)
            {
                ...
                restExcl = excluirReserva(cpf, voo, assentosReservados);//Função utilizada para realizar a exclusao de reservas.
